...
const AdminUpdateAuthor = ({match})=>{
  ...
  const [authorContext, setAuthorContext] = useContext(AuthorContext); 
    ...
    const updateAuthorContext = (newAuthor) =>{
      ...
      const index = authorContextClone.findIndex((a)=>
        a.id === author.id);
    ...
    return(
        <div id="main-content">
            <div className="main_container" style={mainStyle}>
                <h3 style={h3style}>Cập nhật tác giả</h3>
                <form onSubmit={handleSubmit(onSubmit)} >
                    <div class="form-group">
                        <label htmlFor="authorName">Tên tác giả:</label>
                        <input {...register("authorname")} placeholder="Nhập tên tác giả" id="authorName" 
                                className="form-control" defaultValue={author?.authorName}/>
                        <p style={style}>{errors.authorname?.message}</p>
                    </div>
                    <div class="form-group">
                        <label htmlFor="authorInfo">Thông tin tác giả:</label>
                        <textarea {...register("authorinfo")} placeholder="Nhập thông tin tác giả " id="authorInfo" 
                                className="form-control" rows="5" defaultValue={author?.authorInfo}/>
                    </div>
