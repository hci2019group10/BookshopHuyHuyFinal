  ...
  const [optionSearchs, setOptionSearch] = useState([]);
    ...
    const autoComplete = (e) => {
        ...
        const options = preOrder.filter((order) => {
            ...
            .toLowerCase()
            .includes(e.target.value.toLowerCase()) || order.id == e.target.value.toLowerCase();
        });
    ...
    const listOrder = orders.map((order)=>
        <tr>
            ...
            <td><Link to={`/admin/orders/orderDetail/${order.id}`}><i className="fa fa-eye"></i></Link></td>
            <td className="uneditable">
                <span>
                    <Link to={`/admin/orders/UpdateOrder/${order.id}`} style={linkStyle}><i class="fa fa-edit"></i></Link>
                </span>
            </td>
        </tr>
    );
                        ...
                        <div className="row w3-res-tb">
                            ...
                            <div className="col-sm-6">
                                <div className="input-group">
                                    <input type="text" className="input-sm form-control" placeholder="Search" value={search}
                            ...
                            <div className="col-sm-4 m-b-xs">
                                <div className="input-group">
                                    <select className="input-sm form-control w-sm inline v-middle" onChange={sortDate}>
