  ...
  const [categoryContext, setCategoryContext] = useContext(CategoryContext);
   ...
   const categoryID = match.params.categoryID;
    ...
    const updateCategoryContext = (newCategory) =>{
      ...
      const index = categoryContextClone.findIndex((p)=>
        p.id === category.id);
    ...
    return(
        <div id="main-content">
            <div className="main_container" style={mainStyle}>
                <h3 style={h3style}>Thêm thể loại</h3>
                <form onSubmit={handleSubmit(onSubmit)} >
                    <div class="form-group">
                        <label htmlFor="categoryName">Tên thể loại:</label>
                        <input {...register("categoryName")} placeholder="Nhập tên thể loại" id="categoryName" 
                                className="form-control" defaultValue={category?.categoryName}/>
